 ...
 * @version    CVS: $Id: PDFlib.php 253 2007-04-11 02:31:39Z s.moffatt@toowoomba.qld.gov.au $
...
class Image_Graph_Driver_PDFlib extends Image_Graph_Driver
{
    ...
    function _setFont()
    {
        ...
        if (isset($this->_font['name'])) {
            pdf_set_parameter($this->_pdf, 'FontOutline', $this->_font['name'] . '=' . $this->_font['ttf']);
    ...
    function setFont($fontOptions)
    {
        ...
        if (isset($this->_font['ttf'])) {
            if (strtolower(substr($this->_font['ttf'], -4)) != '.ttf') {
                $this->_font['ttf'] .= '.ttf';
            }

            if (file_exists(dirname(__FILE__) . '/../Fonts/' . $this->_font['ttf'])) {
                $this->_font['ttf'] = dirname(__FILE__) . '/../Fonts/' . $this->_font['ttf'];
            } elseif (!file_exists($this->_font['ttf'])) {
                //$this->_font['file'] = false;
            }

            if (!isset($this->_font['name'])) {
                $this->_font['name'] = basename($this->_font['ttf'], '.ttf');
            }
        }
    ...
    function line($x0, $y0, $x1, $y1, $color = false)
    {
        if ($this->_setLineStyle($color)) {
            pdf_moveto($this->_pdf, $this->_getX($x0), $this->_getY($y0));
